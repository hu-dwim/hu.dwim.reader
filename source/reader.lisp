...
"
  (assert (plusp (length (token-text token))))
...
(defun reader-dispatch-macro-vector            (stream arg sub-char)
  ...
  (if arg
      (loop
         :with vector = (make-array arg)
         :for i :from 0 :below arg
         :until (char= #\) (peek-char t stream t nil t))
         :do (setf (aref vector i) (read stream t nil t))
         :finally (progn
                    (if (>= i arg)
                        (let ((*read-suppress* t))
                          ;; TODO: serror "vector is longer than the explicitly given length"
                          (loop
                             :until (char= #\) (peek-char t stream t nil t))
      ...
      (loop
         ...
         :until (char= #\) (peek-char t stream t nil t))
         :do (vector-push-extend (read stream t nil t) vector)
    ...
    (destructuring-bind (expression expected-values expected-error) test
      (multiple-value-bind (actual-values actual-error)
        ...
        (assert (equalp expected-values actual-values)
                () "Testing ~S, expected ~S, got ~S" expression
